...
const utilisateurController = {
 creerUtilisateur: async (req, res) => {
  try {
    ...
    if (role === "medecin") {
      await db.execute(
        `INSERT INTO Medecin (idUtilisateur, specialite, cabinet, disponibilite, tarif_consultation ,heure_ouverture, heure_fermeture)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [result.insertId, specialite, cabinet, disponibilite, tarif_consultation, '08:00', '19:00']
      );
    ...
    try {
      ...
      if (!q || String(q).trim().length < 2) {
        return res
          ...
          .json({
            success: false,
            message: "Paramètre q requis (min 2 caractères)",
          });
      }
      ...
      if (reservations[0].length > 0) {
        return res
          ...
          .json({
            success: false,
            message:
              "Impossible de supprimer l'utilisateur car il a des rendez-vous associés",
          });
      }
